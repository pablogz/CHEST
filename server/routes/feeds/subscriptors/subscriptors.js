...
async function listSubscriptors(req, res) {
    ...
    try {
                    ...
                    if (infoUser.isTeacher) {
                        ...
                        if (idFeed !== null) {
                            ...
                            if (index > -1) {
                                ...
                                // Al cliente le envío una lista de objetos que contrendrá: el identificador del subscriptor, la fecha de subscripción, su alias (si lo tuviera), el número de respuestas que tiene hasta ese momento
                                const out = [];
                                const promesas = [];
                                for (let i = 0, tama = feed.subscriptors.length; i < tama; i++) {
                                    promesas.push(getInfoSubscriptor(feed.subscriptors.at(i), feed.id));
                                }
                                ...
                                winston.info(Mustache.render('listSubscriptors || idUser: {{{idUser}}} - subscriptors: {{{ns}}} || {{{time}}}', {
                                    idUser: uid,
                                    ns: out.length,
                                    time: Date.now() - start
                                }));
                                if (out.length > 0) {
                                    logHttp(req, 200, 'listSubscriptors', start);
                                    res.send(JSON.stringify(out));
                                } else {
                                    logHttp(req, 204, 'listSubscriptors', start);
                                    res.sendStatus(204);
                                }
                            } else {
                                logHttp(req, 404, 'listSubscriptors', start);
                                res.sendStatus(404);
                            }
                        } else {
                            logHttp(req, 400, 'listSubscriptors', start);
                            res.sendStatus(400);
                        }
                    } else {
                        logHttp(req, 401, 'listSubscriptors', start);
                        res.sendStatus(401);
                    }
                } else {
                    logHttp(req, 401, 'listSubscriptors', start);
        ...
        winston.error(Mustache.render(
            'listSubscriptors || {{{error}}} || {{{time}}}',
            {
                error: error,
                time: Date.now() - start
            }
        ));
        logHttp(req, 500, 'listSubscriptors', start);
        res.sendStatus(500);
    }
}

module.exports = { listSubscriptors }